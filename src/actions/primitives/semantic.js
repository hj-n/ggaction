...
function validateDensityTransform(transform) {
  const supported = [
  ...
  validateNonEmptyString(transform.field, "Density field");
  if (transform.groupBy !== undefined) {
    validateNonEmptyString(transform.groupBy, "Density groupBy");
  }
  if (
