...
```text
agent_docs/impl/chart/<chart-name>.md
```
...
- Primitive baseline program
- Public API acceptance test
- Unit tests
- High-resolution PNG regression
- Public tutorial과 API documentation

## 1. 차트 계약 확정

구현 전에 `agent_docs/impl/chart/<chart-name>.md`를 먼저 작성한다. 이 문서는 단순한
...
PNG regression은 파일 생성 여부뿐 아니라 physical dimensions, `pixelRatio`, 실제 ink
pixel, 대표 색상을 검사한다. Browser 검증은 logical Canvas size와 console/page error를
별도로 확인한다.
...
- 일반 test, coverage threshold, PNG regression, CI를 모두 통과시킨다.
...
- [ ] Unit, acceptance, browser, PNG regression이 통과한다.
