...
설계하면서 완성한다. 구현 closeout 뒤에는 npm `0.0.1` public distribution과 이후 release automation까지
완료한다.
...
- [x] Phase 11 — graphical hierarchy와 전체 Planned implementation closeout
- [x] Phase 12 — npm `0.0.1` initial public release, corrective `0.0.2`와 repeatable publishing automation
...
## Phase 11 — Graphical hierarchy and implementation closeout
...
## Phase 12 — Initial public release

- Phase goal: [`phase12/GOAL.md`](phase12/GOAL.md)

완료된 public package를 npm에 `ggaction@0.0.1`로 처음 배포하고 package-boundary correction을 `0.0.2`로 완료한다. Package metadata와 license, minimal tarball,
fresh installed-consumer tests, public installation docs, GitHub tag/release와 secure publishing automation을 하나의
release contract로 검증한다. Gate A에서 legal/visibility/ownership 결정을 확정하고, Gate B에서 exact release
candidate를 승인하기 전에는 실제 registry publish를 실행하지 않는다. First publish 뒤 GitHub-hosted OIDC trusted
publisher와 protected release environment를 최종 상태로 만들어 subsequent release를 같은 절차로 반복한다.
...
- `ggaction@0.0.2`가 npm public registry에서 설치되고 세 public entry와 declaration이 registry consumer에서 통과한다.
- Version, Git tag, GitHub Release, npm dist-tag와 package integrity가 일치한다.
- 이후 release를 protected GitHub/OIDC workflow로 반복할 수 있다.
