...
> **문서 상태 — 현재 실행 계획.** 현재는 active Phase 없이 Phase 12 종료 checkpoint에 머문다.
> 다음 Phase를 여는 결정은 [`ROADMAP_INDEX.json`](../ROADMAP_INDEX.json)이 소유하며, 완료된 Phase의 기록도
> 이 문서에 보존한다.
...
createGradientPlot({ x?, y?, density?, width?, gradient?, center?, ... })
...
- 최소 필수 field만 요구한다. Scatter/line/bar는 `x`, `y`, histogram은 `field`, pre-gridded
  heatmap은 `x`, `y`, `color`, Box/Gradient plot은 하나의 categorical position과 하나의 quantitative
  position, Parallel Coordinates는 `dimensions`가 최소 의미다.
...
| ID | 최소 capability | 실행 Phase 또는 결정 |
| --- | --- | --- |
...
| P-006 | overlay/inset composition | `Maybe Future` — Phase 10 skipped |
| NCP-004 | parallel coordinate resource | 11 |
| P-005 | collision-aware label layout | 12 |
| P-007 | rectangular hierarchy data/layout | `Maybe Future` — Phase 13 skipped |
...
상태 vocabulary는 `planned | in-progress | blocked | completed | skipped`만 사용한다. `skipped`는 구현을
시작하지 않고 범위를 명시적으로 Roadmap 밖의 `Maybe Future`로 옮긴 경우에만 사용한다.
...
| 5 | P5-A / P5-B / P5-C / P5-D / P5-Exit | window / 2D bin / binned heatmap / 통합 |
| 6 | P6-A / P6-B / P6-Exit | gradient paint와 primitive target / gradient plot facade·edit / 통합 |
...
| 10 | — | skipped; P-006은 `Maybe Future`, Gate와 구현 없음 |
...
| 13 | — | skipped; P-007은 `Maybe Future`, Gate와 구현 없음 |
...
Phase 0
  └─ Phase 1: B-002 / B-001 / B-004
       ...
       └─ Phase 7: ordered path (B-004 필요)
            ...
            ├─ Phase 10: skipped; overlay composition → Maybe Future
            └─ Phase 11: parallel coordinate + facade (P-003 helper + B-004 필요)
                 ├─ Phase 12: label layout
                 └─ Phase 13: skipped; hierarchy layout → Maybe Future
                      └─ Phase 14: Basic Chart facade consistency (별도 요청 전 미개방)
...
- B-004는 Phase 7 ordered path와 Phase 11 parallel coordinate의 선행 조건이다.
...
- Shortest direct call은 category와 measure를 x/y에 배치한다. Eligible encoded layer가 있으면
  ...
  수렴한다.
- Category별 raw sample에서 deterministic density profile을 만들고 category당 하나의 ranged rect를 생성한다.
  Profile은 sampled value/intensity와 center를 의미적으로 저장하고 final palette/opacity stop은 graphical
  materialization이 소유한다.
- 기본은 Gaussian, auto bandwidth, shared value extent, 64 samples, unit density, band width `0.7`, sequential
  `blues`와 median center rule이다. Exact parameter shape와 visual defaults는 P6-A에서 승인한다.
- `editGradientPlot`은 density parameter 변경 시 immutable derived revision을 만들고, appearance-only 변경은
  현재 profile revision을 유지하며, center disable/restore는 optional resource를 완전히 제거/재생성한다.
- Facade와 explicit profile/rect/paint/rule chain의 semantic/graphic/order/Canvas-call 동등성을 검증한다.
...
## Phase 10 — Overlay/inset composition (`skipped`)

P-006은 2026-07-21 사용자 결정으로 구현 전에 `Maybe Future`로 이동했다. 현재 핵심 사용 사례가 충분하지
않고 기존 layer, facet과 concat composition으로 우선순위가 높은 요구를 처리할 수 있으므로 package-level
`overlay`, inset placement schema, public types, runtime export, docs와 tests를 추가하지 않는다.

이 결정은 [Phase 10 기록](./phase10/GOAL.md)이 소유한다. 향후 실제 사용 사례와 public contract가
명확해지면 새 Roadmap에서 다시 제안하며, Roadmap 4 Phase 11의 진입 조건으로 취급하지 않는다.

## Phase 11 — Parallel coordinate resource와 Basic Chart facade

### 목표와 포함 ID

NCP-004를 `createCoordinate({ type: "parallel" })`와
`encodeParallelCoordinates({ dimensions, key, ... })`로 구현하고, 이를 감싸는 high-level
`createParallelCoordinates({ dimensions, key?, ... })` facade를 함께 제공한다.

### 의존 관계와 예상 수정 영역

- B-004와 Phase 7 ordered-path helper 완료.
- Coordinate vocabulary/schema, atomic ordered-dimension encoding, dimension-local scale resource/resolution, axis recipe/layout,
...
- High-level facade는 current data에서 coordinate와 line mark를 만들고 `encodeParallelCoordinates`를
  wrapped child로 호출한다. Key 생략은 source lineage row identity를 사용하고 임의의 field를 key로 추론하지 않는다.
...
## Phase 13 — Rectangular hierarchy data/layout (`skipped`)

P-007은 2026-07-21 사용자 결정으로 구현 전에 `Maybe Future`로 이동했다. Representative hierarchy dataset과
treemap/partition product requirement가 확정되지 않은 상태에서 source schema, rollup policy와 layout algorithm을
public contract로 고정하지 않는다. `createHierarchyData`, hierarchy state, public types, primitive/example/Gate를
추가하지 않으며 기존 rect/2D-bin/composition 동작도 변경하지 않는다.

이 결정은 [Phase 13 기록](./phase13/GOAL.md)이 소유한다. 실제 사용 사례가 제시되면 새 Roadmap 또는 별도 명시
요청에서 후보 계약부터 다시 검토한다. Phase 14는 이 skip으로 자동 시작하지 않는다.
