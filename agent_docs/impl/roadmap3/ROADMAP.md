...
- [x] Phase 12 — Source refactor, organization transfer와 `0.0.4` release
...
## Phase 12 — Source refactor, organization transfer, and `0.0.4`

- 공개 동작을 바꾸지 않는 `src/` ownership·dependency 리팩터링
- 새 `ggaction` GitHub organization 생성 후 `ggaction/ggaction`으로 repository transfer
- 일반 push와 public documentation deployment를 분리하고 release tag만 site를 갱신하도록 전환
- 새 repository identity, trusted publishing, Pages와 exact tarball을 검증한 뒤 `ggaction@0.0.4` publish

Phase goal과 Gate는 [`phase12/GOAL.md`](phase12/GOAL.md)에서 관리한다.
