...
| `compositionSpec` | Composition direction, ordered child IDs, gap, alignment와 padding intent |
...
Ordinary unit program은 frozen empty `children`과 absent `compositionSpec`을 가진다. Composition parent는
최소 두 child program과 그 key에 정확히 대응하는 ordered child ID를 저장하며 child semantic state를
parent semantic state로 병합하지 않는다. Child program은 이미 immutable하므로 lookup만 structural copy하고
각 program reference는 그대로 보존한다.
