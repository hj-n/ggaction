...
Chainable `facet({ field })`은 complete unit program을 composition parent로 전환한다. Phase 7 first slice는 모든
eligible layer가 사용하는 하나의 direct source를 먼저 확정하고, field value를 source first-appearance order로
해석한다. 각 cell은 source를 보존한 채 namespaced filtered derived dataset을 만들고 repeated layer를 명시적으로
rebind한 immutable child program이다. Raw facet value는 header text에만 사용하고 generated dataset, child,
graphic ID에는 포함하지 않는다.

Facet은 shared scale domain을 모든 child에 적용하고 histogram이면 template에서 한 번 결정한 bin boundary도
공유한다. Child mark와 guide는 shared scale로 rematerialize한 뒤 parent가 namespaced child Canvas snapshot을
...
- SVG renderer
- derived transform replay, independent scales, outer-only axes와 non-categorical shared legend를 포함한 advanced facet
