...
Low-level semantic primitive는 resolved interval provenance도 검증해 저장할 수 있다. Interval transform은
input field, ordered `groupBy`, `mean | median` center, `stderr | stdev | ci | iqr` extent, CI level과
distinct center/lower/upper output fields를 기록한다. Median은 IQR과만, CI level은 CI extent와만
호환된다. 이 schema support는 primitive oracle을 위한 current behavior이며 `createIntervalData` 계산
action과 public interval materialization은 아직 Planned다.
