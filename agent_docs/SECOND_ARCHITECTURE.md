...
현재 Polar vertical slice는 point와 line mark를 지원한다. Theta의 public 단위는 degree이며 12시 방향의 0에서
...
type이나 angle을 해석하지 않는다. 한 Polar channel만 있는 incomplete state는 semantic으로 보존하되 visible
point 또는 line geometry를 만들지 않는다.

Polar guide는 theta/radius encoding과 동일한 resolved scale 및 Polar frame을 사용한다. Theta axis는 outer
circle, radial axis는 center-to-edge line을 소유하며 theta grid는 spokes, radial grid는 concentric paths를
...
- Polar arc mark materialization
