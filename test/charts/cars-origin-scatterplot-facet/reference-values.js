...
export function createDirectFacetValues(cars) {
  assertCars(cars);
  ...
  if (origins.join("\u0000") !== ORIGINS.join("\u0000")) {
    throw new Error("Cars Origin order changed from the direct facet fixture.");
  }
  ...
  if (cylinders.join("\u0000") !== CYLINDERS.join("\u0000")) {
    throw new Error("Cars Cylinders order changed from the direct facet fixture.");
  }
